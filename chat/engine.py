...
class ChatEngine:
    ...
    def get_all_rooms(self) -> dict[str, list]:
        ...
        return dict(
            private=sorted(private, key=lambda x: x['_last_activity']) if private.__len__() > 0 else private,
            group=sorted(group, key=lambda x: x['_last_activity']) if group.__len__() > 0 else group
        )
